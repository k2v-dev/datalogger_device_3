...
static void cpt_lsm9ds1_I2cEventHandler(nrf_drv_twi_evt_t const * pEvent, void * pContext)
{
    switch(pEvent->type)
    {
 ...
 */
static uint32_t cpt_lsm9ds1_Init(uint8_t sclPin, uint8_t sdaPin)
{
    uint32_t errCode=0;
    ...
    errCode = nrf_drv_twi_init(&m_I2cInstance , &twi_LSM9DS1_config, cpt_lsm9ds1_I2cEventHandler, NULL);
 ...
 **/
static void cpt_lsm9ds1_MergeRegisterAndData(uint8_t * newBuffer, uint8_t reg, uint8_t * pData, uint32_t length)
{
    newBuffer[0] = reg;
...
uint32_t cpt_lsm9ds1_WriteRegisters(uint8_t reg, uint8_t * pData, uint32_t length)
{
    ...
    cpt_lsm9ds1_MergeRegisterAndData(i2cTxBuffer, reg, pData, length);

    // Setting up transfer

    nrf_drv_twi_xfer_desc_t xfer_desc; // Setting Up the transfer function with Slave Address
    xfer_desc.address = WHO_AM_I_AG_RSP;
...
uint32_t cpt_lsm9ds1_WriteSingleRegister(uint8_t reg, uint8_t data)
{
    uint32_t errCode=0;
    uint32_t timeout = LSM9DS1_I2C_TIMEOUT;
  ...
  //  for (is=0; is<127;is++)
  //  {
    	errCode = nrf_drv_twi_tx(&m_I2cInstance,WHO_AM_I_AG_RSP, packet, 2, false);
 ...
 */
uint32_t cpt_lsm9ds1_ReadRegisters(uint8_t reg, uint8_t * pData, uint8_t length)
{
    uint32_t errCode=0;
    uint32_t timeout = LSM9DS1_I2C_TIMEOUT;

    errCode = nrf_drv_twi_tx(&m_I2cInstance, WHO_AM_I_AG_RSP, &reg, 1, false);
    if(errCode != NRF_SUCCESS) return errCode;
    while((!g_I2cTxDone) && --timeout);
    if(!timeout) return NRF_ERROR_TIMEOUT;
    g_I2cTxDone = false;
    ...
    errCode = nrf_drv_twi_rx(&m_I2cInstance, WHO_AM_I_AG_RSP, pData, length);
...
uint32_t  cpt_LSM9DS1_IMU_AG_On(uint8_t scl,uint8_t sda,uint8_t odr)
{
    uint32_t errCode1=0;
    uint32_t errCode2=0;
    uint32_t errCode3=0;
	...
	errCode1 = cpt_lsm9ds1_Init(scl,sda);
	//NRF_LOG_INFO("Init1 %d\n",errCode1);
   ...
   // startBit1 |= odr;
    startBit1 = 0x68;
    errCode2 =  cpt_lsm9ds1_WriteSingleRegister(CTRL_REG6_XL,startBit1);
    NRF_LOG_INFO("Init2 %d\n",errCode2);
    if(errCode1 != NRF_SUCCESS) return errCode2;//ACCEL SET ON 16G

    uint8_t startBit2 = 0;
  //  startBit2 |= odr;
    startBit2 = 0x78;
    errCode3 =  cpt_lsm9ds1_WriteSingleRegister(CTRL_REG1_G,startBit2);  //GYRO SET ON 2000dps
    NRF_LOG_INFO("Init3 %d\n",errCode3);
    NRF_LOG_INFO("REGISTER3");
    if(errCode3 != NRF_SUCCESS) return errCode3;
    return NRF_SUCCESS;
}
/*
uint32_t intcode=1;
